...
void ChangeSensitivity(uint8_t sensitivity){
	...
	if (gOptions & Force3Kg) {
		SetCalibratedSensitivity(FORCE_3KGF);
	} else 	if (gOptions & Force4Kg) {
...
// void FccSettingsHold(uint32_t Buttons){
// 	// COnfig Options for the stick
// 	// TODO: add reset of MIN/MAX values to be Deadzone +1 for full min/max calibration
// 	if (Buttons & (GripMissleStep | GripPaddle)) {
// 		if (gConfigTimer == 0) {
// 			gConfigTimer = millis();
// 		}
// 		//MslStp and paddle are pressed.
// 		if (gIsConfig) { // if we are in config mode
// 			gPowerLed -= 2; // will dim PC7 every round, to have a pulsing light to indicate config mode
// 			setStatusLed(gPowerLed);
...
// 				ChangeSensitivity(Force4Kg);
// 			}
// 			} else { //both are held, but we are not in config mode
// 			if (millis() - gConfigTimer >= 3000) {
// 				gIsConfig = true;
// 			}
// 		}
// 		} else { // if buttons are not held
// 		if (gIsConfig) { //but we are in config mode.
// 			// means that we are now leaving config mode
// 			WriteMem(); // Save all the changes to EEPROM
// 			gIsConfig = false;
// 		}
// 		gConfigTimer = 0;
